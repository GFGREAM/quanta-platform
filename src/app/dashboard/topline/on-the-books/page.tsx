...
function computeMetrics(days: GridDay[], compare: CompareBasis, cap26: number, cap25: number): ColMetrics {
  ...
  for (const d of days) {
    ...
    // (both RN and revenue, derived from D360's rn/rev_change_vs_ly on the 2026 axis).
      ...
      revB += d.isPace ? d.revStly : d.revLy;
    }
    if (d.pickupW != null) { pwSum += d.pickupW; pwHas = true; }
    if (d.pickup4w != null) { p4wSum += d.pickup4w; p4wHas = true; }
  }
  // All comparison bases except Forecast carry both RN and revenue (STLY revenue is derived
  // from D360's rev_change_vs_ly), so Rev/ADR/RevPAR have a basis for budget, ly and stly.
  ...
  // hotel had no 2025 history before it opened (Jan–Mar). Surface "—" across the comparison,
  // variance and variance% rows instead of a 0 base that yields a misleading variance.
